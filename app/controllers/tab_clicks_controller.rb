  ...
  class TabClicksController < ApplicationController
    ...
    def create
      ...
      if @tab_click.save
        redirect_to tab_clicks_path, notice: create_successful_notice
      else
    ...
    def update
      if @tab_click.update(tab_click_params)
        redirect_to tab_clicks_path, notice: update_successful_notice
      else
    ...
    def destroy
      @tab_click.destroy
      redirect_to tab_clicks_path, notice: destroy_successful_notice
    end
    ...
    def set_tab_click
      @tab_click = policy_scope(@restaurant.tab_clicks).find(params[:id])
    end
